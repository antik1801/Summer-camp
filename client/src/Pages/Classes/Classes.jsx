  ...
  const handleAddToCart = (item) => {
    ...
    if (user && user.email) {
      ...
      fetch(`https://medlife-server-navy.vercel.app/carts`, {
        ...
        .then((data) => {
          // fetch (1 course)
          // destructure course.students = course.students + 1
          fetch(`https://medlife-server-navy.vercel.app/bookSeat/${item._id}`)
          .then(res=>res.json())
          .then(data=>{
            const {students} = data;
            const newStudents = students + 1;
            // fetch(`https://medlife-server-navy.vercel.app/bookSeats/${item._id}`,{
            //   method:"PATCH",
            // })
            // .then(res=>res.json())
            // .then(data=>{
      ...
      {approvedCourses.map((course) => (
        ...
        >
          <figure className="px-10 pt-10">
            <img src={course?.picture} alt="Shoes" className="rounded-xl max-h-[300px]" />
          </figure>
